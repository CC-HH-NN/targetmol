"""TargetMol workflow input preparation, planning, and execution."""
...
@dataclass
class WorkflowPlan:
    """Complete step plan for one run."""
...
def _sanitize_env_for_provenance(env: dict[str, str] | None) -> dict[str, str]:
    """Redact environment values before writing provenance records."""
    if not env:
        return {}
    sanitized = {}
...
def _serialize_step_for_provenance(step: WorkflowStep) -> dict[str, object]:
    """Convert step objects into a safely serializable structure."""
    data = asdict(step)
...
def _pick_screening_output(plan: WorkflowPlan) -> str | None:
    """Find the main screening output path for summaries."""
...
def _write_run_summary_metadata(
    *,
    ...
    """Write shared summary context into provenance."""
...
def create_run_dir(base_dir: Path, run_name: str, timestamp: str | None = None) -> Path:
    """Create the directory for one run."""
...
def resolve_input_spec_for_run(config: TargetMolConfig, spec: InputSpec, run_dir: Path) -> InputSpec:
    """Resolve remote inputs into local executable inputs before planning."""
...
def plan_workflow(config: TargetMolConfig, spec: InputSpec, run_dir: Path) -> WorkflowPlan:
    """Build an execution plan from inputs and configuration."""
    ...
    if route == "ligand_based_targetmol":
        raise ValueError("The ligand_based_targetmol route requires candidate SMILES generated by the CLI.")
    raise ValueError(f"Unknown workflow route: {route}")


def write_plan_snapshot(plan: WorkflowPlan, output_path: Path) -> Path:
    """Write the workflow plan to a JSON file."""
...
def execute_plan(
    ...
    """Execute workflow steps sequentially and write the execution index."""
    execution_index = []
...
def _plan_screen_only(config: TargetMolConfig, spec: InputSpec, run_dir: Path) -> WorkflowPlan:
    """Build a plan for direct screening of existing candidate SMILES."""
    screening_dir = run_dir / "screening"
    if spec.pdb_file is None:
        raise ValueError("The screen_only route requires an input that resolves to a local PDB file.")
...
def _plan_targetmol_ligand_screening(config: TargetMolConfig, spec: InputSpec, run_dir: Path) -> WorkflowPlan:
    """Build a plan that sends TargetMol ligand candidates into internal screening."""
    screening_dir = run_dir / "screening"
    if spec.pdb_file is None or spec.candidate_smiles_file is None:
        raise ValueError("The ligand_based_targetmol screening path requires a local PDB and generated candidates.")
    command = _build_internal_screening_command(
        conda_env=config.envs.screening_conda_env,
        normalized_smiles_file=spec.candidate_smiles_file,
        receptor_pdb=spec.pdb_file,
...
def _plan_targetmol_ligand_candidates_only(spec: InputSpec, run_dir: Path) -> WorkflowPlan:
    """Build a plan for ligand candidates when structure-based screening is unavailable."""
    outputs = []
...
def _validate_screen_only_spec(spec: InputSpec) -> None:
    """Reject generation inputs that conflict with screen-only execution."""
    conflicting_inputs = []
    ...
    if conflicting_inputs:
        ...
        raise ValueError(f"Candidate files cannot be combined with generation inputs: {joined}")


def _is_targetmol_generated_candidate_spec(spec: InputSpec) -> bool:
    """Return whether a candidate file was generated by the TargetMol ligand route."""
...
def _plan_sbdd_drugflow(config: TargetMolConfig, spec: InputSpec, run_dir: Path, route: str) -> WorkflowPlan:
    """Build a plan for the DrugFlow structure-based route."""
    if spec.pdb_file is None or spec.reference_ligand is None:
        raise ValueError("The sbdd_drugflow route requires both a local PDB and a reference ligand.")
...
def _build_internal_screening_command(
    *,
    ...
    """Build the internal screening runner command."""
    command = [
        "conda",
        "run",
        "-n",
